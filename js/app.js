  ...
  const app = new Vue({
    ...
    methods: {
      ...
      touchmove(event) {
        ...
        this.cursor.x = Math.min(Math.max(0, this.touch.cursorX + Math.floor(movedX / 10)), this.width-1);
        this.cursor.y = Math.min(Math.max(0, this.touch.cursorY + Math.floor(movedY / 10)), this.height-1);
      },
      touchend(event){
        this.touch = {};
      },
